...
public class PluginManager
{
	...
	public void loadPlugins()
	{
		...
		registrar.completeRegistration(Spade.main.tools, Spade.main.effects);
	}
	...
	private boolean validatePlugin(Plugin plugin)
	{
		...
		{
			if(Version.parse(info.get("min-spade-version")).isGreater(Spade.getVersion()))
			{
				System.err.println("[PluginManager] Plugin \"" + name + "\" requires a newer version of Spade");
				System.err.println("[PluginManager] Have: " + Spade.getVersion());
				System.err.println("[PluginManager] Need at least: " + Version.parse(info.get("min-spade-version")));
				plugins.add(plugin);
				return false;
			}
		}
		...
		{
			if(Spade.getVersion().isGreater(Version.parse(info.get("max-spade-version"))))
			{
				System.err.println("[PluginManager] Plugin \"" + name + "\" requires an older version of Spade");
				System.err.println("[PluginManager] Have: " + Spade.getVersion());
				System.err.println("[PluginManager] Need less than: " + Version.parse(info.get("max-spade-version")));
				plugins.add(plugin);
				return false;
			}
		}
