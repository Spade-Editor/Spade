...
import heroesgrave.spade.image.change.doc.MetadataChange;
		...
		done.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent arg0)
			{
				if(!layer.getMetadata().get("name").equals(label.getText()))
					Spade.getDocument().addChange(new MetadataChange(layer, "name", label.getText()));
				if(blendMode.getSelectedItem() != null)
					if(!layer.getMetadata().get("blend").equals(blendMode.getSelectedItem()))
						Spade.getDocument().addChange(new MetadataChange(layer, "blend", (String) blendMode.getSelectedItem()));
				dialog.setVisible(false);
			}
		});
