...
public class Renderer extends JPanel
{
	...
	public Renderer(Document doc)
	{
		this.doc = doc;
		this.revalidateBG();
		this.image = new BufferedImage(doc.getWidth(), doc.getHeight(), BufferedImage.TYPE_INT_ARGB);
	}
	...
	public void revalidateBG()
	{
		...
		bgPaint =
				new TexturePaint(Menu.DARK_BACKGROUND ? transparencyBGDark : transparencyBG, new Rectangle2D.Float(0,
						0, 16, 16));
		this.repaint();
	}
	...
	public void paint(Graphics $g)
	{
		...
		if(Menu.GRID_ENABLED && scale >= 4)
		{
			...
			{
				g.drawLine(MathUtils.floor(i * scale), 0, MathUtils.floor(i * scale),
						MathUtils.floor(doc.getHeight() * scale));
			}
			for(int j = 0; j < doc.getHeight(); j++)
			{
				g.drawLine(0, MathUtils.floor(j * scale), MathUtils.floor(doc.getWidth() * scale),
	...
	static
	{
		try
		{
			transparencyBG = ImageIO.read(Renderer.class.getResource("/heroesgrave/paint/res/tbg.png"));
			
			transparencyBGDark = ImageIO.read(Renderer.class.getResource("/heroesgrave/paint/res/tbgd.png"));
		}
		catch(IOException e)
		{
			e.printStackTrace();
